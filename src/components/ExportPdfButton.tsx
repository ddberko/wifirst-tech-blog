  ...
  const handleExport = useCallback(async () => {
    ...
    try {
      ...
      header.innerHTML = `
        ...
        ${finalCoverImage ? `<img src="${finalCoverImage}" crossorigin="anonymous" style="width: 100%; max-height: 280px; object-fit: cover; border-radius: 12px; margin-bottom: 20px;" />` : ""}
      ...
      content.querySelectorAll("pre").forEach((el) => {
        el.style.cssText = "background: #1e1e2e; color: #e0e0e0; padding: 16px; border-radius: 8px; font-size: 12px; overflow: hidden; page-break-inside: avoid; margin: 16px 0; white-space: pre-wrap; word-break: break-word;";
      });
      content.querySelectorAll("code").forEach((el) => {
        if (el.parentElement?.tagName !== "PRE") {
          el.style.cssText = "background: #f3f4f6; padding: 2px 6px; border-radius: 4px; font-size: 13px; color: #d63384;";
        }
      });
      content.querySelectorAll("blockquote").forEach((el) => {
        el.style.cssText = "border-left: 4px solid #0066CC; background: #f0f7ff; padding: 12px 16px; margin: 16px 0; border-radius: 0 8px 8px 0; page-break-inside: avoid;";
      });
      content.querySelectorAll("img").forEach((el) => {
        el.style.cssText = "max-width: 100%; border-radius: 8px; margin: 16px 0; page-break-inside: avoid;";
        ...
        if (src.startsWith("/_next/image")) {
          try {
            ...
            if (originalUrl) src = originalUrl;
          } catch { }
        }
      ...
      content.querySelectorAll("table").forEach((el) => {
        el.style.cssText = "width: 100%; border-collapse: collapse; margin: 16px 0; font-size: 13px; page-break-inside: avoid;";
      });
      content.querySelectorAll("th").forEach((el) => {
        el.style.cssText = "background: #f8f9fa; padding: 8px 12px; border: 1px solid #e5e7eb; font-weight: 600; text-align: left;";
      });
      content.querySelectorAll("td").forEach((el) => {
        el.style.cssText = "padding: 8px 12px; border: 1px solid #e5e7eb;";
      });
      ...
      footer.style.cssText = "border-top: 2px solid #e5e7eb; margin-top: 40px; padding-top: 16px; text-align: center; font-size: 11px; color: #999;";
      footer.innerHTML = `
            ...
            new Promise<void>((resolve) => {
              if (img.complete && img.naturalWidth > 0) {
                ...
                img.onerror = () => {
                   ...
                   resolve(); // Resolve anyway so PDF generates
                };
              }
            })
        )
      ...
      await new Promise((resolve) => setTimeout(resolve, 500)); // Slightly longer delay for paint
